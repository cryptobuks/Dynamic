...
void GetNameList(const CNameVal& nameUniq, std::map<CNameVal, NameTxInfo>& mapNames, std::map<CNameVal, NameTxInfo>& mapPending)
{
    ...
    for (const std::pair<CNameVal, std::set<uint256>> &item : mapNamePending)
    {
        if (!item.second.size())
            continue;
...
UniValue name_debug(const UniValue& params, bool fHelp)
{
    ...
    {
        LOCK(cs_main);
        for (const std::pair<CNameVal, std::set<uint256>> &pairPending : mapNamePending)
        {
            std::string name = stringFromNameVal(pairPending.first);
            LogPrintf("%s :\n", name);
            uint256 hash;
            for (auto hash : pairPending.second)
            {
                LogPrintf("    ");
...
bool createNameScript(CScript& nameScript, const CNameVal& name, const CNameVal& value, int nRentalDays, int op, std::string& err_msg)
{
    ...
    // split value in 520 bytes chunks and add it to script
    {
        ...
        {   // insert data
            ...
            std::vector<unsigned char>::const_iterator sliceEnd = std::min(value.begin() + (i+1)*520, value.end());
